...
Trataremos siempre trabajar con el paquete lubridate que es el estándar más moderno y más utilizado actualmente. En la ayuda podrán ver las funciones de este paquete.
...
paste("wday(hoy):", wday(hoy)) 
paste("mday(hoy):", mday(hoy)) 
paste("day(hoy):", day(hoy)) 
paste("yday(hoy):", yday(hoy)) 
paste("month(hoy):", month(hoy))
paste("week(hoy):", week(hoy)) 
paste("year(hoy):", year(hoy)) 
```

Un ejemplo pavo de uso:

Calcula el tiempo que pasó entre dos fechas.

```{r}
days(today() - dmy("23-08-70"))
```
...
Los vectores son conjuntos ordenados de objetos de la misma clase (o type, se averigua con typeof()). Se instancian con c(). Pueden contener cualquier tipo de objetos mientras sean de la misma clase y los fuerza a la misma clase por defecto. Las operaciones básicas funcionan elemento a elemento.
...
vec1 <- c(1:10) # Desde : hasta También se puede 1:10 , pero no se recomienda.
...
> Importante: en R los índices comienzan en 1, eso es diferente a python que comienzan en 0
...
### Multiplicación de matrices elemento a elemento (el operador es *).
...
### Multiplicaciones algebraicas de matrices (el operador es %*%).

Tenga en cuenta que se necesitan las dimensiones correctas para la multiplicación de matrices. El número de filas de la primera debe ser igual al número de columnas de la segunda.
...
### Producto Kronecker (el operador es %x%).

Este producto hace todas las multiplicaciones posibles de los elementos de la primera matriz con los de la segunda. En este caso cada matriz tiene 6 elementos, por eso el producto tiene 6 * 6 = 36 elementos.
...
En una lista los objetos pueden nombrarse. Esto se parece a los diccionarios de python
...
## Data Frames. Heterogéneos de 2D

Aviso: en el curso usaremos una estructura similar llamada tibble (del paquete tidyverse). Aquí se explican los dataframes porque son la estructura por defecto de R, en la próxima clase se abordarán los tibbles más exensamente.

> Atención!: Puede tener diferentes tipos de datos en diferentes columnas (variables), pero dentro de la columna los tipos de datos deben ser homogéneos.
...
```{r}
unDataFrame1[[1]] # Toda la columna 1
writeLines("")
unDataFrame1$factor # Toda la columna factor
writeLines("")
unDataFrame1[[4,3]] # fila, columna
writeLines("")
unDataFrame1[[3]][4] # Toda la columna 3 y luego dentro de ella extraigo el elemento 4
writeLines("")
unDataFrame1[[1]][2] # Toda la columna 1 y luego dentro de ella extraigo el elemento 2
writeLines("")
unDataFrame1$cadena[3]  # Toda la columna cadena y luego dentro de ella extraigo el elemento 3
```
...
Largo (mm), ancho (mm), cantidad de folíolos (cantidad), largo del pecíolo (mm).
...
```{r}
# Sumatoria
# Itera sobre los elemntos de un vector directamente
suma <- 0 
for (i in largos) {
  suma <- suma + i
}
suma
# Itera los índices de un vector y accede a los elementos a través del índice. En R todo está optimizzadopara trabajar como en el ejemplo anterior, directamente sobre los objetos del vector, lista, matriz, array, etc.
suma2 <- 0 
for (i in 1:length(largos)) {
  suma2 <- suma2 + largos[i]
}
suma2
# length , largo del vector (cantidad de elementos que lo componen)
n <- 0 
for (i in largos) {
  n <- n + 1
}
n
# media o promedio de todos los valores del vector
media <- suma / n
media
...
```{r}
variables <- variable.names(dfHojas)[1:4]
tibbleHojas <- dfHojas %>% pivot_longer(variables, names_to = "Variable", values_to = "Valor")
tibbleHojas
```
...
### Una mejor alternativa, mire los ejes y.

```{r}
tibbleHojas <- tibbleHojas %>% group_by(tibbleHojas$Variable)
grafico <- ggplot(tibbleHojas) +
  aes(x= Variable, y = Valor) +
  geom_boxplot() +
  stat_boxplot(geom = "errorbar",
