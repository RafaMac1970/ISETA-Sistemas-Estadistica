...
Los "tibble" son los "data.frame" de "tidyverse". Los tibbles pueden tener una estructura que no sea tidy, pero toda la lógica de trabajo es con tidy, de modo que no hacerlo hace muchas tareas más engorrosas. Además hay una función pivot_wider(), que permitehacer lo contrario de pivot_longer(), es devir, tomar los valores de una columna y usarlos como nombrede nuevas columnas (rara vez es necesario, pero siempre existe la alternativa).
...
En este ejemplo usamos dataframes que ya existe en R, que se llaman iris y cars. iris y cars son lo que llamamos "built-in dataset", más detalles en "Extra-Built-in datasets.Rmd".
...
Nótese que usa los nombres de los vectores como nombre de cada columna.

### 4. Crear una matriz y luego convertirla en tibble.

```{r}
matriz2 <- matrix(1:16, ncol = 4)
unTibble3 <- tibble(data.frame(matriz2))
unTibble3
```
...
Hay diferencias importantes con un dataframe respecto del acceso a información. Aquí va una comparación de algunos casos de uso común. La tabla muestra la clase del objeto que devuelve cada comando.
...
Vamos a trabajar con los built-in dataset mtcars y diamonds.
