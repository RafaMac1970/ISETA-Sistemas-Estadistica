...
Es el conjunto de todos los resultados que puede producir un evento aleatorio, los eventos son subconjuntos del espacio muestral. Lo representamos con las llaves -igual que la representación de un conjunto- {}.
...
$$
p(D)= \frac{\#(D)}{\#\{M\}} = \frac{3}{6} = \frac{1}{2}
$$
...
# sample() Selecciona al azar desde un vector. Se puede indicar que lo seleccionado vuelva al vector inicial antes de hacer la siguiente selección (replace = TRUE), o que el vector se achique cada vez que le saca un elemento (replace = FALSE).
...
```{r}
resultados <- permutations(n = 10, r = 3, v = data_scientists)
nrow(resultados)
resultados
```
...
```{r}
resultados <- combinations(n = 10, r = 2, v = data_scientists)
nrow(resultados)
resultados
```
