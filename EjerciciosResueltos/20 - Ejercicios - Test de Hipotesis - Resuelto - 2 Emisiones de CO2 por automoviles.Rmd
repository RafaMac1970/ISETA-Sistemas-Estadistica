...
```{r}
rango = c(min(mu_01 - 5 * error_estandar1, media_muestral1), mu_01 + 1 * error_estandar1) # Esto establece el rango de graficado en el eje x

ggplot(data.frame(x = rango), aes(x = rango)) +
  stat_function(fun = dnorm, n = 101, args = list(mean = media_muestral1, sd = error_estandar1)) +
  geom_area(stat = 'function', fun = dnorm, args = list(mean = media_muestral1, sd = error_estandar1), fill = '#ff0018', xlim = c(rango[2], mu_01), alpha = 0.5) +
  geom_vline(xintercept = media_muestral1) +
  geom_vline(xintercept = mu_01) +
  geom_label(aes(x = media_muestral1, y = -1, label = round(media_muestral1,3)))  +
  ylab("Densidad") + ggtitle("Distribución Normal") + xlab("Emisiones de CO2 (g/km)") +
  theme_classic()
```
