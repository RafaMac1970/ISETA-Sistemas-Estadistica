...
Se realizó un muestreo de los tiempos de reacción de cuatro individuos. Para esto una persona (sujeto) apoyó el brazo sobre una mesa con su mano asomando sobre uno de suscostados. Otra persona (aplicador) sostuvo una regla de 30cm de largo de modo que el 0 (cero) de la regla quede entre los dedos pulgar e índice del sujeto. El aplicador suelta la regla sin previo aviso y el sujeto debe detener la caída de la regla presionando sus dedos sobre la misma. Este procedimiento se realizó 30 (treinta) veces seguidas a cada sujeto.
...
t es el momento en el que realizamos en cálculo. En nuestro caso es la incógnita, es el tiempo (medido en s) que el sujeto tarda en detener la caída de la regla. Por lo tanto tenemos que despejarlo en la ecuación.
...
# Esta línea permite descartar datos extremos que posiblemente sean distorsivos. Esta decisión debe tomarse luego de mirar la distribución de los datos crudos.
...
indicesReflejosDist <- data_tidy %>% 
  group_by(NOMBRE) %>% 
  reframe(n = n(), media = mean(DISTANCIA), moda = moda(DISTANCIA), mediana = median(DISTANCIA), sd = sd(DISTANCIA), varianza = var(DISTANCIA), cv = sd/media, ee = sd/(sqrt(n)), icMin = media - (1.96 * ee), icMax =  media + (1.96 * ee))
indicesReflejosDist
```
...
indicesReflejosTiempo <- data_tidy %>% 
  group_by(NOMBRE) %>% 
  reframe(n = n(), media = mean(TIEMPO), moda = moda(TIEMPO), mediana = median(TIEMPO), sd = sd(TIEMPO), varianza = var(TIEMPO), cv = sd/media, ee = sd/(sqrt(n)), icMin = media - (1.96 * ee), icMax =  media + (1.96 * ee))
...
media1 <- indicesReflejosTiempo[[mayor,3]]
media2 <- indicesReflejosTiempo[[menor,3]]
sd1 <- indicesReflejosTiempo[[mayor,6]]
sd2 <- indicesReflejosTiempo[[menor,6]]
...
Lauta <- rnorm(n = 10000, mean = 0.1914155, sd =	0.009860397)
Mario <- rnorm(n = 10000, mean = 0.1472720, sd =	0.042211931)
Nacho <- rnorm(n = 10000, mean = 0.2066651, sd =	0.015721508)
Rafa <- rnorm(n = 10000, mean = 0.2110530, sd =	0.016835476)
...
```{r}
media1 <- indicesReflejosDist[[mayor,3]]
media2 <- indicesReflejosDist[[menor,3]]
sd1 <- indicesReflejosDist[[mayor,6]]
sd2 <- indicesReflejosDist[[menor,6]]
alfa1 <- qnorm(0.05, media1, sd1)
alfa2 <-qnorm(0.95, media2, sd2)
